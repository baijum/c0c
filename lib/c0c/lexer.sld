                ...
                ((char=? ch #\/)
                 (let ((nxt (peek)))
                   (cond
                     ...
                     ((and (not (eof-object? nxt)) (char=? nxt #\*))
                      (advance!)
                      (if (and (not (eof-object? (peek))) (char=? (peek) #\@))
                          (begin (advance!) (scan-annotation line col))
                          (begin (skip-block-comment line col) (loop))))
                     ((and (not (eof-object? nxt)) (char=? nxt #\=))
                ...
                ((char=? ch #\@)
                 (let ((nxt (peek)))
                   (if (and (not (eof-object? nxt)) (char=? nxt #\*))
                       (begin (advance!)
                              (let ((nxt2 (peek)))
                                (when (and (not (eof-object? nxt2)) (char=? nxt2 #\/))
                                  (advance!)))
                              (loop))
                       (error "c0c: unexpected @" filename line col))))
                (else
